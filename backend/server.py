...
async def enhanced_calculate_requirements(request: EnhancedCalculationRequest):
    try:
        ...
        for _, row in grouped.iterrows():
            ...
            # New formula: Quantité à Envoyer = max(0, (CQM x JOURS A COUVRIR) - Stock Actuel)
            # For basic calculation (no transit stock): quantity_to_send = max(0, (request.days * adc) - current_stock)
            ...
            # Apply new formula: quantity_to_send = max(0, (CQM x JOURS A COUVRIR) - Stock Transit - Stock Actuel)
            # quantity_to_send = max(0, required_stock - transit_available - current_stock)
            ...
            if request.inventory_session_id and request.inventory_session_id in inventory_data:
                ...
                result_item['inventory_status_text'] = 'DONNÉES MANQUANTES'
                result_item['inventory_status_color'] = 'text-blue-600 bg-blue-50'
